...
bool logIn(string name, string pass) {
    ...
    //p->execute("python.exe",QStringList() << "data/classify.py");
    //p->waitForFinished();
    ...
    //qDebug() << QString::fromStdString(name) << QString::fromStdString(pass);
    ...
    qDebug() << "Client : connect successfully!"<< endl;
	memset(recvbuf, 0, MAXBUF);

	//发送账号
	int numUser = send(httpsock, name.c_str(), strlen(name.c_str()), 0);
	recv(httpsock, recvbuf, MAXBUF, 0);
    qDebug() << "Client : send account"<< endl;
    if (numUser == strlen(name.c_str()))
        ;
	else {
        qDebug() << "Account Failed" << endl;
        closesocket(httpsock);
        WSACleanup();
        return false;
	}
	...
	recv(httpsock, recvbuf, MAXBUF, 0);
    qDebug() << "Client : send password" << endl;
    if (numPass == strlen(pass.c_str()))
        qDebug() << "Password Success" << endl;
	else {
        qDebug() << "Password Failed" << endl;
        closesocket(httpsock);
        WSACleanup();
        return false;
	}
    ...
    while (count <= mailNumber) {        
        ...
        //qDebug() << QString::fromLocal8Bit(text.c_str()) << QString::fromStdString(date);
...
void pickUpInfo(char *s, string *from, string *subject, string *text, string *date) {
    ...
    regex textPattern("base64(\\s)+([0-9A-Za-z\\=/+]+)");
